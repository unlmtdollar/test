...
async function refreshContractInfo() {
  ...
  try {
    ...
    document.getElementById("currentPhase").textContent =
      `Phase ${currentPhaseId} - ${currentPhaseName}`;
...
async function withdrawSDA() {
  ...
  if (!amount || amount <= 0) return showStatus("Enter a valid amount", "error");
...
async function updateDevWallet() {
  ...
  if (!ethers.isAddress(newWallet)) return showStatus("Enter valid wallet", "error");
...
async function updatePhase() {
  ...
  if (!phaseId || !phaseName || !rate) return showStatus("Fill all fields", "error");

  try {
    showStatus("Updating phase...", "info");
    const tx = await contract.updatePhase(Number(phaseId), phaseName, Number(rate));
    await tx.wait();
...
window.addEventListener("load", () => {
  if (typeof ethers === "undefined") {
    showStatus("Ethers.js not loaded. Check CDN.", "error");
    return;
  }
  if (typeof window.ethereum !== "undefined") {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (withdrawInput) {
    withdrawInput.addEventListener("keypress", (e) => { if (e.key === "Enter") withdrawSDA(); });
  }
  const newWalletInput = document.getElementById("newDevWallet");
  if (newWalletInput) {
    newWalletInput.addEventListener("keypress", (e) => { if (e.key === "Enter") updateDevWallet(); });
  }
});
